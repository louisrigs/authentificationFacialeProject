...
class MyTestCase(unittest.TestCase):
	...
	def test_auth_brute(self):
		...
		self.assertEqual(auth(load.probes1[index],load.gallery), True)
		print(bfs.radius_search(load.gallery, load.probes2[index], radius)[0])
		self.assertEqual(auth(load.probes2[index],load.gallery), False)
		print(bfs.radius_search(load.groundtruth, load.probes2[index], radius)[0])
		self.assertEqual(auth(load.probes2[index],load.groundtruth), True)
		print(bfs.radius_search(load.groundtruth, load.probes1[index], radius)[0])
		self.assertEqual(auth(load.probes1[index], load.groundtruth), True)
